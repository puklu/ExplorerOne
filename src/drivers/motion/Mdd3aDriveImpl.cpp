...
Mdd3aDrive::Mdd3aDrive(std::unique_ptr<Motor> pFrontRightMotor,
...
int8_t Mdd3aDrive::CalculateSpeedForSlowerSide(eTurnRadius turn_radius)
{
    ...
    switch (turn_radius)
    {
        case eTurnRadius::NORMAL:
            speed_for_slow_moving_side =
                static_cast<float>(mCurrentSpeedPercent) * 0.35f;
            break;

        case eTurnRadius::LARGE:
            speed_for_slow_moving_side =
                static_cast<float>(mCurrentSpeedPercent) * 0.75f;
            break;

        case eTurnRadius::MEDIUM:
            speed_for_slow_moving_side =
                static_cast<float>(mCurrentSpeedPercent) * 0.55f;
            break;

        case eTurnRadius::SMALL:
            speed_for_slow_moving_side =
                static_cast<float>(mCurrentSpeedPercent) * 0.10f;
            break;

        case eTurnRadius::ZERO:
            speed_for_slow_moving_side =
                static_cast<float>(mCurrentSpeedPercent) * -1.0f;
            break;
    }
...
{
    int8_t speed_for_slow_moving_side =
        CalculateSpeedForSlowerSide(turn_radius);
