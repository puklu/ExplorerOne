 ...
 *
 * This class provides an interface for configuring, starting, stopping,
 * resetting, and handling interrupts for basic timers. It extends the `ITimer`
 * interface and interacts with STM32F303 hardware timers.
 */
class BasicTimer : public BaseTimer
{
   public:
     ...
     *
     * @param timer_config Reference to the BasicTimerConfig containing
     * configuration.
     */
     ...
     *
     * Returns the function pointer to the interrupt callback that is triggered
   ...
   private:
     ...
     *
     * Configures the necessary control registers to enable auto-reload, update
     * requests, and starts the timer.
     *
     * @return `eGeneralStatus::SUCCESS` if the operation is successful,
     * otherwise an error status.
     */
     ...
     *
     * Manually triggers an update event which reloads the configuration
     * settings.
     */
