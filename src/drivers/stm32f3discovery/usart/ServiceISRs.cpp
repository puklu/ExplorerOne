...
void UsartServiceISR_Printing()
{
    ...
    if ((usart->ISR & (USART_ISR_TXE)))  // if the interrupt telling that
                                         // transmit data register is active.
    {
        char nextData;
        if (activePrintUsartPin->GetRingBuffer()->take(nextData) ==
        ...
        {
            activePrintUsartPin
                ->DisableTxRegisterEmptyInterrupt();  // if there is no data to
                                                      // transmit, disable the
                                                      // interrupt which tells
                                                      // that transmit data
                                                      // register is empty.
        }
    }
