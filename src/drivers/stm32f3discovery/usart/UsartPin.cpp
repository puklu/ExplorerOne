...
std::shared_ptr<UsartPin> UsartPin::Create(
...
UsartPin::UsartPin(UsartPinInitStruct const &pin_init_struct)
    : PinBase(pin_init_struct.pin_name),
      ...
      mpRingBuffer(
          std::make_shared<RingBuffer>(RING_BUFFER_SIZE, mCriticalSectionGuard))
{
}
...
void UsartPin::TransmitData(char data)
{
    ...
    if (mpRingBuffer->put(data) !=
        eRingBufferStatus::RING_BUFFER_STATUS_SUCCESS)
    {
        ASSERT(0);
    }
...
void UsartPin::SetAlternateFunction()
{
    ...
    if (mPinNumber < 8)
    {
        ...
        mpPort->AFR[0] |=
            (static_cast<uint8_t>(mAlternateFunction) << (mPinNumber * 4));
    }
    else
    {
        ...
        mpPort->AFR[1] |=
            (static_cast<uint8_t>(mAlternateFunction) << (pin_number * 4));
    }

    // TODO: Replace the above code by something like below to use the set
    // method of the pin. GpioPin *pin =
    // static_cast<GpioPin*>(activePins[mPortNumber][mPinNumber]);
...
void UsartPin::SelectUsart()
{
    ...
    switch (mPortNumber)
    {
        case 0:
            selectedUsart =
                aAltFunctionsAdressesPortA[mPinNumber][static_cast<uint8_t>(
                    mAlternateFunction)];
            break;

        case 1:
            selectedUsart =
                aAltFunctionsAdressesPortB[mPinNumber][static_cast<uint8_t>(
                    mAlternateFunction)];
            break;

        case 2:
            selectedUsart =
                aAltFunctionsAdressesPortC[mPinNumber][static_cast<uint8_t>(
                    mAlternateFunction)];
            break;

        case 3:
            selectedUsart =
                aAltFunctionsAdressesPortD[mPinNumber][static_cast<uint8_t>(
                    mAlternateFunction)];
            break;

        case 4:
            selectedUsart =
                aAltFunctionsAdressesPortE[mPinNumber][static_cast<uint8_t>(
                    mAlternateFunction)];
            break;

        case 5:
            selectedUsart =
                aAltFunctionsAdressesPortF[mPinNumber][static_cast<uint8_t>(
                    mAlternateFunction)];
            break;
    ...
    {
        mpUsart = const_cast<USART_TypeDef *>(
            reinterpret_cast<const USART_TypeDef *>(selectedUsart));
    }
    else
    {
        ASSERT(0);
...
void UsartPin::SetControlRegister()
{
    ...
    else if (mOversamplingMode ==
             USART::eOverSamplingMode::USART_OVERSAMPLING_BY_8)
    {
        mpUsart->CR1 |= (1 << 15);
    }
    ...
    if (mParityControl ==
        USART::eParityControlEnable::USART_PARITY_CONTROL_ENABLED)
    {
        mpUsart->CR1 |= (1 << 10);
    }
    else if (mParityControl ==
...
uint32_t UsartPin::GetInterruptAndStatusRegister(
    USART::eIsrFlags const &flag) const
{
...
{
    if (flag != USART::eIcrFlags::RESERVED5 &&
        flag != USART::eIcrFlags::RESERVED7 &&
        flag != USART::eIcrFlags::RESERVED10 &&
        flag != USART::eIcrFlags::RESERVED13 &&
        flag != USART::eIcrFlags::RESERVED14 &&
        flag != USART::eIcrFlags::RESERVED15 &&
        flag != USART::eIcrFlags::RESERVED16 &&
        flag != USART::eIcrFlags::RESERVED17 &&
...
void UsartPin::GetIRQn()
{
    ...
    {
        ASSERT(0);
        mIrqNumber = NonMaskableInt_IRQn;  // TODO: Fix me. return appopriate
                                           // code or handle in some other way
    }
}
