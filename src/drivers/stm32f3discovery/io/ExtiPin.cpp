...
std::shared_ptr<ExtiPin> ExtiPin::Create(
...
/**
For the external interrupt lines, to generate the interrupt, the interrupt line
should be configured and enabled. This is done by programming the two trigger
registers with the desired edge detection and by enabling the interrupt request
by writing a ‘1’ to the corresponding bit in the interrupt mask register. When
the selected edge occurs on the external interrupt line, an interrupt request is
generated. The pending bit corresponding to the interrupt line is also set. This
request is reset by writing a 1 in the pending register. For the internal
interrupt lines, the active edge is always the rising edge. The interrupt is
enabled by default in the interrupt mask register and there is no corresponding
pending bit in the pending register. For the external lines, an interrupt/event
request can also be generated by software by writing a 1 in the software
...
the EXTI so that an interrupt coming from one of the EXTI lines can be correctly
acknowledged.

Any of the external lines can be configured as software interrupt/event lines.
The following is the procedure to generate a software interrupt.
...
void ExtiPin::EnableInterrupt(InterruptCallback cb)
{
    ...
    mpSystemConfigController->EXTICR[mPinNumber / 4] |=
        IO::aSyscfgExtiRegisterBits[mPinNumber];
    mpSystemConfigController->EXTICR[mPinNumber / 4] &=
        (mPortNumber << (4 * (mPinNumber % 4)));

    // Unmask the Interrupt mask register bit for the EXTI line. EXTI line
    // number corresponds to the pin number so this works out
