...
{
    uint8_t                  portIdx = 0;
    std::shared_ptr<ExtiPin> pin     = nullptr;

    for (portIdx = 0; portIdx < IO_PORT_COUNT; portIdx++)
    {
        // TODO: Assert that pin type in ExtiPin type
        pin =
            std::dynamic_pointer_cast<ExtiPin>(activeExtiPins[portIdx][pinNum]);
...
void ExtiServiceISR5_15(uint8_t startPinNumber, uint8_t endPinNumber)
{
    ...
    for (portIdx = 0; portIdx < IO_PORT_COUNT; portIdx++)
    {
        ...
        {
            // TODO: Assert that pin type in ExtiPin type
            pin = std::dynamic_pointer_cast<ExtiPin>(
                activeExtiPins[portIdx][pinIdx]);
            if (pin != nullptr && pin->isInterruptPresent())
            {
                ExtiPin::InterruptCallback callback =
                    pin->GetInterruptCallback();
                if (callback != nullptr)
                {
                    callback();
                }
