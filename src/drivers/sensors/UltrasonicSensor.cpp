...
UltrasonicSensor::UltrasonicSensor(std::shared_ptr<ITimer>      timer,
...
void UltrasonicSensor::SendTriggerPulse()
{
    ...
    while (mpTimer->GetTimeElapsedInMicrosecondsSinceStart() - startTime <
           10_us)
        ;
...
void UltrasonicSensor::WaitForEchoToGoHigh()
{
    ...
    {
        // TRACE_LOG("mpEchoPin->ReadInputValue(): %d",
        // mpEchoPin->ReadInputValue());
    }
    // TRACE_LOG("mpEchoPin->ReadInputValue(): %d",
    // mpEchoPin->ReadInputValue());
}
