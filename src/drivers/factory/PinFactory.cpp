...
std::shared_ptr<PinBase> PinFactory::CreatePin(
    IO::ePinType pin_type, const PinBaseInitStruct &pin_init_struct)
{
    // Get the port number and pin number
    uint8_t port_num =
        (pin_init_struct.pin_name & IO_PORT_MASK) >> IO_PORT_OFFSET;
    ...
    switch (pin_type)
    {
        case IO::ePinType::IO_PIN_TYPE_GPIO:
            pin = GpioPin::Create(static_cast<const GpioPinInitStruct &>(
                pin_init_struct));  // TODO: Try to get rid of casting. might be
                                    // losing information?
            break;
        case IO::ePinType::IO_PIN_TYPE_EXTI:
            pin = ExtiPin::Create(static_cast<const ExtiPinInitStruct &>(
                pin_init_struct));  // TODO: Try to get rid of casting. might be
                                    // losing information?
        ...
        {
            auto usart_pin =
                UsartPin::Create(static_cast<const UsartPinInitStruct &>(
                    pin_init_struct));  // TODO: Try to get rid of casting.
                                        // might be losing information?
