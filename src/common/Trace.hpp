/**
 * @file Trace.hpp
 * @brief Provides a logging utility for debugging and tracing information
 * through USART.
 *
 * The `Trace` class enables formatted output to a USART pin for debugging
 * purposes. It supports logging with optional metadata (module names) and
 * file/line details for tracing.
 *
 * Usage:
...
#define TRACE_LOG(format, ...) \
    Trace::printWithDetails(__FILE__, __LINE__, format, ##__VA_ARGS__)
...
#define TRACE(module, format, ...) \
    Trace::printWithMetadata(module, format, ##__VA_ARGS__)
...
#define TRACE_LOG(format, ...)                                              \
    [](const auto&...) {}(__VA_ARGS__), (void)(__FILE__), (void)(__LINE__), \
        (void)0
#define TRACE(module, format, ...)
#define PRINT(format, ...)
 ...
 *
 * The `Trace` class provides methods for logging with different levels of
 * detail, including simple logs, logs with module names, and detailed logs with
...
class Trace
{
   public:
     ...
     *
     * This sets up the USART pin for logging. Should be called once before any
     * logging.
     */
   ...
   private:
    static bool isInitialized; /*!< Flag to indicate if the Trace class is
                                  initialized. */
};
